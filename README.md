...
WebDAV 代理桥接服务，提供元数据缓存、速率限制、符号链接功能和现代化 WebUI 管理界面，适用于视频/音频流媒体场景。
...
| **WebUI** | 现代化 Web 管理界面，支持文件浏览、配置管理、状态监控 |
| **JWT 认证** | 单用户 JWT + Refresh Token 认证，保护 WebUI API |
| **WebSocket** | 实时状态推送，文件操作事件即时通知 |
| **暗黑模式** | 支持暗黑/明亮模式切换，跟随系统主题 |
...
WebdavBridge 内置现代化 WebUI 管理界面，基于 React + TypeScript + TailwindCSS 构建。

### 功能概览

| 功能 | 说明 |
|------|------|
| **文件浏览** | 目录列表、面包屑导航、上传/下载/删除操作 |
| **配置管理** | 在线编辑上游连接、缓存、速率限制等配置项 |
| **状态监控** | 查看缓存统计、WebSocket 实时事件流 |
| **认证** | JWT 登录认证，自动 Token 刷新 |
| **暗黑模式** | 暗黑/明亮主题切换，支持系统主题检测 |

### 访问方式

启动 WebdavBridge 后，访问 `http://<server_bind>/` 即可进入 WebUI。

默认登录凭据（可通过环境变量修改）：
- 用户名：`admin`（`WEBUI_USERNAME`）
- 密码：`admin`（`WEBUI_PASSWORD`）
...
| `/config` | 配置管理页面 |
| `/status` | 状态监控页面 |
| `/webdav/*` | WebDAV 端点（兼容标准 WebDAV 客户端） |
| `/api/*` | REST API 端点 |
| `/ws` | WebSocket 实时状态 |
| `/health` | 健康检查 |
...
| 变量 | 默认值 | 说明 |
|------|--------|------|
| `WEBUI_USERNAME` | `admin` | WebUI 登录用户名 |
| `WEBUI_PASSWORD` | `admin` | WebUI 登录密码 |
| `JWT_SECRET` | 随机生成 | JWT 访问令牌密钥 |
| `JWT_REFRESH_SECRET` | 随机生成 | JWT 刷新令牌密钥 |
...
│       ├── lib/
│       │   ├── api.ts            # Axios API 客户端（JWT 自动刷新）
│       │   ├── theme.ts          # 暗黑/明亮模式管理
│       │   └── useWebSocket.ts   # WebSocket Hook（自动重连）
│       ├── components/
│       │   └── Layout.tsx        # 布局组件（导航栏、主题切换）
│       └── pages/
│           ├── LoginPage.tsx     # 登录页面
│           ├── FilesPage.tsx     # 文件浏览器
│           ├── ConfigPage.tsx    # 配置管理
...
└── src/
    ...
    ├── webui/
    │   ├── mod.rs                # WebUI 模块导出
    │   ├── router.rs             # Axum 路由构建（WebDAV + API + 静态文件）
    │   ├── state.rs              # 共享应用状态（AppState）
    │   ├── auth.rs               # JWT 认证（登录/登出/刷新/中间件）
    │   ├── ws.rs                 # WebSocket 实时状态推送
    │   └── api/
    │       ├── mod.rs            # API 模块导出
    │       ├── config.rs         # 配置 API（GET/PUT /api/config）
    │       ├── status.rs         # 状态 API（GET /api/status）
    │       ├── files.rs          # 文件操作 API（浏览/上传/下载/删除/复制/移动）
    │       └── symlinks.rs       # 符号链接 API（列表/创建/删除/更新）
    ├── cache/
    │   ├── mod.rs                # 缓存模块导出
...
| 类别 | 依赖 | 说明 |
|------|------|------|
| Web 框架 | axum 0.8, tower, tower-http | Axum HTTP 服务器（路由、中间件、静态文件） |
| HTTP 服务器 | hyper 1, http 1, hyper-util | 底层 HTTP/1.1 实现 |
...
| 认证 | jsonwebtoken 9, bcrypt 0.17 | JWT 令牌 + 密码哈希 |
| WebSocket | tokio-tungstenite | WebSocket 支持 |
| 前端嵌入 | rust-embed 8 | 编译时嵌入前端资源 |
...
| `config` | 2 | 默认配置、保存/加载 |
| `tasks` | 2 | 任务创建 |
| `webui::auth` | 4 | JWT 创建/验证、过期令牌拒绝、令牌类型检查 |

### 添加新功能

1. 在 `src/webdav/types.rs` 中定义新的数据类型或错误变体
2. 在 `src/webdav/server.rs` 中实现新的处理器方法
3. 在 `src/webui/router.rs` 中添加路由
4. 在 `src/webui/api/` 中添加 API 端点
5. 在 `frontend/src/` 中添加前端页面
6. 编写单元测试
